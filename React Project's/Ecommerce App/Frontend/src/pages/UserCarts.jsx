...
const UserCarts = () => {
  ...
  let totalAmount = 0;
  const renderCarts = localCart.map((c) => (
    totalAmount += c.product.price * c.quantity,
    <div className="flex items-center justify-between mt-5" key={c.product.id}>
      <div className="w-[40%] flex p-1">
        <img className="w-30 h-30 rounded-xl" src={c.product.images[0]} alt={c.product.title} />
        <div className="ml-8 p-3 flex flex-col justify-between">
          <h1 className="text-[var(--text-h)] capitalize">{c.product.title}</h1>
          <h2 className="text-[var(--text-p)] capitalize pb-2">{c.product.category}</h2>
        </div>
      </div>
      ...
      <div className="w-[24%] flex items-center justify-center gap-5">
        <button
  ...
  return (
    <div className="w-full h-[90vh] p-7">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="cursor-pointer px-3 py-3 flex gap-2 items-center justify-center bg-gray-900 rounded-md hover:scale-99 active:scale-98"
        >
      ...
      <div className="flex items-center justify-between mt-10">
        <div className="w-[40%] text-md capitalize text-[var(--text-p)] text-center">product</div>
        <div className="w-[18%] text-md capitalize text-[var(--text-p)] text-center">price</div>
        <div className="w-[24%] text-md capitalize text-[var(--text-p)] text-center">qty</div>
        <div className="w-[18%] text-md capitalize text-[var(--text-p)] text-center">total</div>
      </div>
      ...
      <div className="flex flex-col gap-4 w-full mt-5">
        {localCart.length > 0 ? (
          ...
          <h2 className="text-center text-2xl text-[var(--text-p)] mt-10">Your cart is empty</h2>
        )}
      </div>

        {totalAmount == 0 ? "" :  <div className="w-[95%] h-[12%] bg-[var(--text-p)] fixed bottom-5 left-8 rounded-2xl flex items-center justify-around">
        ...
        <div className="w-[20%] h-full flex flex-col items-center justify-center">
          ...
          <h2 className="text-xl">$ {(totalAmount - totalAmount/10).toFixed(2)}</h2>
        </div>
        <button className="w-[25%] p-3 bg-[var(--btn)] text-2xl rounded-xl hover:scale-99 active:scale-98 cursor-pointer ">Buy Now</button>
      </div>}
