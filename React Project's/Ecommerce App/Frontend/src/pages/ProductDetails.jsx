 ...
 const productSlider =  product?.images?.map((c, i) => (
  ...
  return (
    <div className="w-full h-fit overflow-auto flex flex-col">
      <div className="w-full h-[78vh] flex justify-between p-7">
        <div className=" w-[40%] h-[100%] px-3">
        <Slider {...settings} className="h-full w-full">
           {productSlider}
        </Slider>
        </div>

        <div className="w-[57%] h-full pt-5 flex flex-col justify-between">
      ...
      {users?.isAdmin && (
        <div className="p-5 w-[100%] h-fit">
          <button
            onClick={() => setshow(!show)}
            className="w-[100%] p-4 capitalize text-3xl bg-[var(--btn)] rounded-2xl font-bold tracking-widest hover:scale-99 active:scale-98 "
          >
